...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    private static int randomHashSeed(ConcurrentHashMap instance) {
        if (sun.misc.VM.isBooted() && ConcurrentHashMap.Holder.ALTERNATIVE_HASHING) {
    ...
    static final class HashEntry<K,V> {
        ...
        static {
            try {
                ...
                Class k = ConcurrentHashMap.HashEntry.class;
    ...
    static final <K,V> ConcurrentHashMap.HashEntry<K,V> entryAt(ConcurrentHashMap.HashEntry<K,V>[] tab, int i) {
        return (tab == null) ? null :
                (ConcurrentHashMap.HashEntry<K,V>) UNSAFE.getObjectVolatile
     ...
     */
    static final <K,V> void setEntryAt(ConcurrentHashMap.HashEntry<K,V>[] tab, int i,
                                       ConcurrentHashMap.HashEntry<K,V> e) {
    ...
    static final class Segment<K,V> extends ReentrantLock implements Serializable {
        ...
        private void scanAndLock(Object key, int hash) {
            ...
            int retries = -1;
            while (!tryLock()) {
                ConcurrentHashMap.HashEntry<K,V> f;
                if (retries < 0) {
     ...
     */
    @SuppressWarnings("unchecked")
    static final <K,V> ConcurrentHashMap.Segment<K,V> segmentAt(ConcurrentHashMap.Segment<K,V>[] ss, int j) {
        long u = (j << SSHIFT) + SBASE;
        return ss == null ? null :
                (ConcurrentHashMap.Segment<K,V>) UNSAFE.getObjectVolatile(ss, u);
    }
    ...
    private ConcurrentHashMap.Segment<K,V> ensureSegment(int k) {
        final ConcurrentHashMap.Segment<K,V>[] ss = this.segments;
        long u = (k << SSHIFT) + SBASE; // raw offset
        ConcurrentHashMap.Segment<K,V> seg;
        if ((seg = (ConcurrentHashMap.Segment<K,V>)UNSAFE.getObjectVolatile(ss, u)) == null) {
            ConcurrentHashMap.Segment<K,V> proto = ss[0]; // use segment 0 as prototype
            int cap = proto.table.length;
            float lf = proto.loadFactor;
            int threshold = (int)(cap * lf);
            ConcurrentHashMap.HashEntry<K,V>[] tab = (ConcurrentHashMap.HashEntry<K,V>[])new ConcurrentHashMap.HashEntry[cap];
            if ((seg = (ConcurrentHashMap.Segment<K,V>)UNSAFE.getObjectVolatile(ss, u))
                    == null) { // recheck
                ConcurrentHashMap.Segment<K,V> s = new ConcurrentHashMap.Segment<K,V>(lf, threshold, tab);
     ...
     */
    @SuppressWarnings("unchecked")
    private ConcurrentHashMap.Segment<K,V> segmentForHash(int h) {
        long u = (((h >>> segmentShift) & segmentMask) << SSHIFT) + SBASE;
        return (ConcurrentHashMap.Segment<K,V>) UNSAFE.getObjectVolatile(segments, u);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    static final <K,V> ConcurrentHashMap.HashEntry<K,V> entryForHash(ConcurrentHashMap.Segment<K,V> seg, int h) {
    ...
    public boolean containsKey(Object key) {
        ConcurrentHashMap.Segment<K,V> s; // same as get() except no need for volatile value read
        ConcurrentHashMap.HashEntry<K,V>[] tab;
        int h = hash(key);
        long u = (((h >>> segmentShift) & segmentMask) << SSHIFT) + SBASE;
        if ((s = (ConcurrentHashMap.Segment<K,V>)UNSAFE.getObjectVolatile(segments, u)) != null &&
    ...
    public V putIfAbsent(K key, V value) {
        ...
        int hash = hash(key);
        int j = (hash >>> segmentShift) & segmentMask;
        if ((s = (ConcurrentHashMap.Segment<K,V>)UNSAFE.getObject
    ...
    public V remove(Object key) {
        int hash = hash(key);
        ConcurrentHashMap.Segment<K,V> s = segmentForHash(hash);
    ...
    public void clear() {
        final ConcurrentHashMap.Segment<K,V>[] segments = this.segments;
        for (int j = 0; j < segments.length; ++j) {
            ConcurrentHashMap.Segment<K,V> s = segmentAt(segments, j);
            if (s != null)
                s.clear();
        }
    }
    ...
    public Set<K> keySet() {
        Set<K> ks = keySet;
        return (ks != null) ? ks : (keySet = new ConcurrentHashMap.KeySet());
    }
    ...
    public Collection<V> values() {
        ...
        return (vs != null) ? vs : (values = new ConcurrentHashMap.Values());
    }
    ...
    public Set<Map.Entry<K,V>> entrySet() {
        ...
        return (es != null) ? es : (entrySet = new ConcurrentHashMap.EntrySet());
    }
    ...
    abstract class HashIterator {
        ...
        final void advance() {
            for (;;) {
                if (nextTableIndex >= 0) {
                    ...
                    ConcurrentHashMap.Segment<K,V> seg = segmentAt(segments, nextSegmentIndex--);
        ...
        final ConcurrentHashMap.HashEntry<K,V> nextEntry() {
            ConcurrentHashMap.HashEntry<K,V> e = nextEntry;
            if (e == null)
            ...
            implements Iterator<Entry<K,V>>
    {
        public Map.Entry<K,V> next() {
            ConcurrentHashMap.HashEntry<K,V> e = super.nextEntry();
            return new ConcurrentHashMap.WriteThroughEntry(e.key, e.value);
        }
    }
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        public Iterator<Map.Entry<K,V>> iterator() {
            return new ConcurrentHashMap.EntryIterator();
        }
        public boolean contains(Object o) {
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        final ConcurrentHashMap.Segment<K,V>[] segments = this.segments;
        for (int k = 0; k < segments.length; ++k) {
            ConcurrentHashMap.Segment<K,V> seg = segmentAt(segments, k);
            seg.lock();
            try {
                ConcurrentHashMap.HashEntry<K,V>[] tab = seg.table;
                for (int i = 0; i < tab.length; ++i) {
                    ConcurrentHashMap.HashEntry<K,V> e;
                    for (e = entryAt(tab, i); e != null; e = e.next) {
                        s.writeObject(e.key);
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        final ConcurrentHashMap.Segment<K,V>[] oisSegments = (ConcurrentHashMap.Segment<K,V>[])oisFields.get("segments", null);
        ...
        final ConcurrentHashMap.Segment<K,V>[] segments = this.segments;
        for (int k = 0; k < segments.length; ++k) {
            ConcurrentHashMap.Segment<K,V> seg = segments[k];
            if (seg != null) {
                seg.threshold = (int)(cap * seg.loadFactor);
                seg.table = (ConcurrentHashMap.HashEntry<K,V>[]) new ConcurrentHashMap.HashEntry[cap];
            }
        }
    ...
    static {
        ...
        try {
            UNSAFE = sun.misc.Unsafe.getUnsafe();
            Class tc = ConcurrentHashMap.HashEntry[].class;
            Class sc = ConcurrentHashMap.Segment[].class;
